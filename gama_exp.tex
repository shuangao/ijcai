...
\textbf{Source+1:} This method only train a new binary LS-SVM for the new category. For the rest of the classes, we use the predictions of the classifiers trained from source data directly. %This is arguably the easiest way for transfer learning. 
...
\begin{figure}
\centering
\includegraphics[scale=0.3]{fig/A2A_gama.eps} %\label{fig:awa-b}
...
Still we take the experiment where horse is considered as the new category to see the detail performance of each algorithm. % and show the it in Figure \ref{fig:awa} . 
...
\begin{figure}
	\centering
	 \includegraphics[scale=0.3]{fig/A2C_gama.eps} %\label{fig:a2c-b}
